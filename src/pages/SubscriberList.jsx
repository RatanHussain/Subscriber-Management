...
export default function SubscriberList() {
  ...
  const [newSubscriber, setNewSubscriber] = useState({ name: '', phone: '', startDate: '', discounts: {}, payments: {} });
  ...
  const handlePaymentChange = async (subscriberId, value) => {
    ...
    const currentPayments = docData.payments || {};
    const alreadyPaid = !!currentPayments[currentMonthKey];

    const updatedPayments = { ...currentPayments, [currentMonthKey]: Number(value) };
    const updatedPaidMonths = alreadyPaid
      ? docData.paidMonths
      : (docData.paidMonths || 0) + 1;
  ...
  const handleAddSubscriber = async (e) => {
    ...
    setNewSubscriber({ name: '', phone: '', startDate: '', discounts: {}, payments: {} });
    // window.location.reload();
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const list = snapshot.docs.map(docSnap => {
        ...
        const message = totalDue > monthlyFee
          ...
          : `Your WiFi bill expired in ${formatDate(nextDueDate)}. Please pay ${monthlyFee} riyal as soon as possible.`;
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow space-y-6">
      ...
      <form onSubmit={handleAddSubscriber} className="grid grid-cols-1 sm:grid-cols-6 gap-4 bg-gray-50 p-4 rounded">
        <input
          type="text"
          placeholder="Name"
        ...
        <input
          ...
          onChange={(e) => setNewSubscriber({
            ...newSubscriber,
            discounts: { ...newSubscriber.discounts, [currentMonthKey]: Number(e.target.value) || 0 }
          })}
          className="border p-2 rounded"
        />
        <input
          ...
          onChange={(e) => setNewSubscriber({
            ...newSubscriber,
            payments: { ...newSubscriber.payments, [currentMonthKey]: Number(e.target.value) || 0 }
          })}
          className="border p-2 rounded"
        />
            ...
            <tr key={i} className={sub.due ? 'bg-red-50' : 'bg-green-50'}>
              ...
              <td className="p-2">
                <input
                  ...
                  onChange={(e) => setSubscribers(prev => {
                    const copy = [...prev];
                    copy[i].discounts[currentMonthKey] = Number(e.target.value) || 0;
                    return copy;
                  })}
                />
              </td>
              <td className="p-2">
                <input
                  ...
                  onChange={(e) => setSubscribers(prev => {
                    const copy = [...prev];
                    copy[i].payments[currentMonthKey] = Number(e.target.value) || 0;
                    return copy;
                  })}
                />
              </td>
